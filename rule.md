...
### ASIS（現状）
- 対象時間: 8:00〜14:00 のチェックが朝チェックとして認識される
- 期限超過後（8:51以降）にチェックしてもOK扱いになる
- ダッシュボードに「朝チェックなし」が正しく表示されない
...
### ASIS（現状）
- 対象時間: 14:00以降のすべてのチェックが午後チェックとして認識される
- 時間制限がなく、夕方以降のチェックも午後チェック扱い
...
```python
# 現在（誤り）
CLOSED_DAYS: str = "0"  # 0=Sunday, 6=Saturday

# 正しくは
CLOSED_DAYS: str = "0"  # 0=Monday, 6=Sunday
```

**影響**: 現在の設定 `"0"` は**月曜日**が休診日として扱われる。
