...
def create_check(
    ...
    if prev_check:
        delta = current_time - prev_check.checked_at
    ...
    for idx, img in enumerate(images):
        ...
        # Create DB record
        # Store relative path or full path? Let's store relative path from storage root for portability if needed, 
        # but architecture says full path. Let's stick to full path for now or relative to a base.
        # Architecture says: /var/data/toilet-images/YYYY/MM/DD/{check_id}/...
        # Let's store the full path as per spec.
